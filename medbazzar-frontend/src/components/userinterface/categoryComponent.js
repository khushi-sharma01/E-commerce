...
export default function CategoryComponent(props) {
  ...
  const showCategorySlideShimmer = () => {
    return ["", "", "", "", "", "", ""]?.map((item) => {
      return (
        <div>
          <Skeleton height={180} width={180} animation="wave">
            <div
              style={{
                display: "flex",
                marginLeft: 12,
  ...
  const box = () => {
    return category?.map((category, index) => (
        <div key={index}>
          <img
            src={`${serverURL}/images/${category.picture}`}
            style={{
              width: "95%",
          ...
          />
          <div style={{display:'flex',alignItems:'center',justifyContent:'center'}}>{category.categoryname}</div>
        </div>
      ));
  };
  const handleForward=()=>{
...
<ArrowBackIosIcon onClick={handleBackward}/>
    ...
    <Slider ref={sld} {...settings}>   {category.length ? box() : showCategorySlideShimmer()}</Slider>
